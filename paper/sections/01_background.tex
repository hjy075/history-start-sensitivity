...
We distinguish three effects. First, the \textbf{local common-support effect} compares two forecasts for the same series, same target time, and same realized target, changing only the history start. Second, the \textbf{eligibility effect} measures how many early forecast origins disappear when a minimum history requirement is applied after trimming. Third, the \textbf{global propagation effect} asks whether local changes are prevalent enough to change benchmark-level scores, rankings, or winners.
...
These results connect intermittent-demand forecasting, retail benchmark design, and forecast measurement. The contribution is not that first-positive trimming is always harmful; the evidence rejects that stronger claim. Instead, history-start conventions are a measurable source of evaluation sensitivity whose local importance depends on model family, data context, and evaluation origin, while aggregate benchmark conclusions may remain robust.
