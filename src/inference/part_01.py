# Inference and paper-facing post-processing
import sys, json
...
def require(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run the Visuelle and FreshRetail experiment notebooks first."
        )
    return path
...
def series_weighted_product_cluster_bootstrap(series_loss, reps=5000, seed=0):
    ...
    point = x.groupby("model", as_index=False).agg(
    ...
    gp = x.groupby(["product_id", "model"], as_index=False).agg(
    ...
    def make_matrix(value):
        return gp.pivot(index="product_id", columns="model", values=value).reindex(
            index=products, columns=models
    ...
    for b in range(reps):
        ...
        wt[it] += 1
        changed += int(ir != it)
        tau_reps[b] = stats.kendalltau(stats.rankdata(mr), stats.rankdata(mt)).statistic

    effects = point.set_index("model").reindex(models).reset_index()[["model", "delta_MAE", "n_series"]]
    ...
    return point.sort_values("rank_release").reset_index(drop=True), effects, winner, tau_summary
...
def effect_comparison(dataset, old_df, new_df):
    ...
    new = new_df.rename(columns={"delta_MAE": "new_delta_MAE", "ci_low": "new_ci_low", "ci_high": "new_ci_high"})
    z = new.merge(old[keep], on="model", how="left")
    z["dataset"] = dataset
    z["delta_estimand_difference"] = z["new_delta_MAE"] - z["old_delta_MAE"]
    return z

estimand_comparison = pd.concat([
    effect_comparison("Visuelle", vis_boot_old, vis_effects),
    effect_comparison("FreshRetail", fresh_boot_old, fresh_effects),
], ignore_index=True)
...
for model, g in fresh_profile.groupby("model"):
    ...
    audit_rows.append({
        ...
        "non_monotonic": bool(not (np.all(np.diff(vals) >= -1e-12) or np.all(np.diff(vals) <= 1e-12))),
    })
